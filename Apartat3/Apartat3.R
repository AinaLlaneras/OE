...
library(dplyr)
rm(list = ls())
#### 3.1. Problema infactible ####

# ampl: include E1A31.run;
# presolve, variable P[7,8,21]:
#   impossible deduced bounds: lower = 8.14, upper = 8;
#   difference = 0.14
#   presolve, variable P[7,8,14]:
#     impossible deduced bounds: lower = 8.58, upper = 8;
#   difference = 0.58
#   presolve, variable P[7,8,13]:
#     impossible deduced bounds: lower = 8.25, upper = 8;
#   difference = 0.25
#   presolve: constraint NPFE[11,21] cannot hold:
#     body <= -75.57 cannot be >= -73; difference = -2.57
#   presolve: constraint NPFE[11,15] cannot hold:
#     body <= -73.04 cannot be >= -73; difference = -0.04
#   13 presolve messages suppressed.
#   CostTotal = 36961.1
...
P <- read.table("P3.txt", header = TRUE)
pg <- read.table("Apartat3_5_PG.txt", header = TRUE)
th <- read.table("Apartat3_5_TH.txt", header = TRUE)
alpha <- read.table("Apartat3_1_alpha.txt", skip = 1)
colnames(alpha) <- c("D03","D04","D05","D08","D09","D10","D11","D12","D13","D14")
u <- read.table("Apartat3_5_u.txt", header = TRUE)
v <- read.table("Apartat3_1_v.txt", header = TRUE)
...
df2 <- read.table("Apartat3_2.txt", skip = 1)
...
T <- 1:24
df3 <- read.table("Apartat3_3.txt", header = TRUE)
df3$T <- T
...
marges <- read.table("Apartat3_42.txt", header = TRUE)
U <- marges$rup
D <- marges$rdown
M <- marges
